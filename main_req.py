...
class MyThread(QThread):
    ...
    def run(self):  # 在启动线程后任务从这个函数里面开始执行
        self.sock.connect(('127.0.0.1', server['server_port_2']))
        print('是否连接2')
        while True:    #不停的接消息
            while True:
...
class Myregister(QtWidgets.QMainWindow, Ui_Form2):
    ...
    def register(self):
        try:
            ...
            if len(nick_name) > 7:
                ...
                box.warning(self, "提示", "密码强度过弱，请加入数字!")
            elif not re.search("[a-zA-Z]", password):
                box.warning(self, "提示", "密码强度过弱，请加入字母!")
            elif not re.search('@', email):
                box.warning(self, '提示', '邮箱格式有误！')
            elif len(ver_code) == 0:
                box.warning(self, '提示', '验证码为空')
    ...
    def send_num(self):
        ...
        if not re.search('@', email):
            ...
            arg = email
            e = client.send_code(arg)
            if e == 0:
                box.information(self, "提示", "验证码发送成功,验证码30秒内将发送到你的邮箱！")
            elif e == 1:
                box.warning(self, "警告", "验证码已发送，请之后在尝试!")
