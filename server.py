...
address = []
File_list = []
PATH = r'D:\com study\course\sotf scurity\chatTools-master\data\tansport'
PATH_2 = r'D:\com study\course\sotf scurity\chatTools-master\data\recieve'
A = 0
    ...
    if a == 0:
        ...
        if b == 0:
            ...
            if c == 0:
                ...
                the_code=reponse['args']['ver_code']
                d= server_mysql.check_code(the_email,the_code)
                if d == 1:
                    print("验证码错误")
                    reg["return"] = 4
                else:
                    server_mysql.delet_code(the_email)
...
def sedn_cd(reponse):
    ...
    reg["login"] = 5
    the_email = reponse["args"]  # 查询数据库
    a = server_mysql.check_code_email(the_email)     #验证邮箱
    if  a == 1:
        reg["return"] = 1
    else:
        threading.Thread(target=smtp_mail_1.main, args=(the_email,)).start()
        reg["return"] = 0
